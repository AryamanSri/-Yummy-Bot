  ...
  async def Yummy(self,ctx):
    async with aiohttp.ClientSession() as r:
      async with r.get("https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=yummy") as r:
        ...
        embed.add_field(name= 'Current Price', value = f'```{data[0]["current_price"]}```', inline = True)
        embed.add_field(name= 'Market Cap', value = f'```{data[0]["market_cap"]}```', inline = True) 
        embed.add_field(name= 'High 24hrs🔼', value = f'```{data[0]["high_24h"]}```', inline = True) 
        embed.add_field(name= 'Low 24hrs 🔽 ', value = f'```{data[0]["low_24h"]}```', inline = True) 
        embed.add_field(name= 'Price Change', value = f'```{data[0]["price_change_24h"]}```', inline = True) 
        embed.add_field(name= 'Price Change %', value = f'```{data[0]["market_cap"]}```', inline = True) 
